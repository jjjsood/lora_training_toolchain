...
Because `convert` never touches FLUX, `train`/`train-matrix` writes the *only* FLUX artifact this toolchain ever produces — `adapter/<ID>.safetensors`, kohya key layout, unconverted — and that is exactly what a caller should point `verify-keys` at. `verify-keys` closes this seam itself: for `arch: flux` it detects a kohya-layout state dict (the same suffix check `lorafactory.survey.introspect` uses) and runs it through the same native `FluxLoraLoaderMixin.lora_state_dict` bridge described above before diffing it against the pinned vocabulary, so the module-name comparison always happens in diffusers layout regardless of which layout the file on disk is in. SD3 gets none of this — its `verify-keys` input must already be `convert`ed, unchanged from before.
